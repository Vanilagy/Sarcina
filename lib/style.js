...
function getCssElements(markupProcessData) {
    ...
    cssElements.push(...getCssLinkElements(markupProcessData)); // It is important we get cssLinkElements first, because that function could also cut out missingCssFileTags, which would then mess with other indexes if we computed it after something else.
    if (markupProcessData.bundler.options.handleInlineCss) cssElements.push(...getStyleElements(markupProcessData));
...
async function getCssBundlers(markupProcessData, stringSplicer) {
    ...
    for (let element of allCssElements) {
        ...
        if (element.isInline) {
            if (!markupProcessData.bundler.options.handleInlineCss) {
                await addBundler();
...
async function addCssElementsToCssBundler(cssElements, cssBundler) {
    for (let element of cssElements) {
        let { isInline, isAbsolute, href, body, file } = element;
...
class CssFileBundler {
    ...
    async addCurrentBundle() {
        ...
        if (reference.isAbsolute) {
            ...
            let bundle = findMatchingCssBundle(this.currentBundleElements, this.markupProcessData.bundler);
            let code;
            ...
            if (bundle) {
                ...
                if (this.markupProcessData.bundler.options.injectCss) {
                    ...
                    bundle = this.markupProcessData.bundler.createBundle('.css', this.currentBundleElements, code, true);
                } else {
                    // Create a non-virtual bundle
                    bundle = this.markupProcessData.bundler.createBundle('.css', this.currentBundleElements, code);
                }
...
function findMatchingCssBundle(elements, bundler) {
