...
class Bundler {
    ...
    createBundle(extension, elements, code, isVirtual = false) {
        /* Virtual bundles won't be written to disk, but are only used internally */
        ...
        let bundle = new Bundle(bundleFile, elements);
...
async function processDirectory(directory, bundler) {
    for (let file of directory.files) {
        ...
        if (file.constructor === fileUtils.File) {
            ...
            if (fileUtils.isMarkupFile(file)) {
