...
class ScriptElement extends markup.HtmlElement {
    constructor(data, markupProcessData) {
        ...
        this.isInline = !this.attributes.has('src');
        ...
        if (this.isInline) this.isDefered = false; // defer is ignored for inline scripts

        this.isAbsolute = util.urlIsAbsolute(this.attributes.get('src') || '');
        this.file = null;

        if (this.attributes.has('src') && !this.isAbsolute) {
...
const POLYFILL_IO_SRC = `https://polyfill.io/v3/polyfill.min.js`;
...
async function processScriptElements(markupProcessData) {
    ...
    if (markupProcessData.bundler.options.insertPolyfill) insertPolyfill(markupProcessData);
}

async function bundleScripts(markupProcessData) {
    let stringSplicer = new util.StringSplicer(markupProcessData.contents);
    let scriptBundlers = await getScriptBundlers(markupProcessData, stringSplicer);
...
function insertPolyfill(markupProcessData) {
    ...
    let stringSplicer = new util.StringSplicer(markupProcessData.contents);
    let insertPosition = options.OptionConstants.START_OF_HEAD;
...
async function getScriptBundlers(markupProcessData, stringSplicer) {
    ...
    for (let element of allScriptElements) {
        ...
        if (element.isInline) {
            if (!markupProcessData.bundler.options.handleInlineScript) {
                await addBundler();
...
// Move all defered elements to the end of the list, as those get executed last. Allows for better bundling.
        ...
        if (reference.isAbsolute) {
            ...
            if (bundle) {
                ...
                if (reference.isModule) {
                    ...
                    if (!reference.attributes.get('src')) {
