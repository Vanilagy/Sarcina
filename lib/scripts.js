...
async function getScriptBundlers(markupProcessData) {
    ...
    while ((match = scriptRegExp.exec(markupProcessData.contents)) !== null) {
        ...
        if (markupProcessData.bundler.options.scriptInsertPosition === options.OptionConstants.SMART) {
            let lastElement = scriptElements[scriptElements.length - 1];
            if (lastElement) {
                if (!markupProcessData.isEmptyInRange(lastElement.match.index + lastElement.match[0].length, match.index)) {
                    await addBundler();
                }
            }
        }        
    ...
    async function addBundler() {
        if (currentBundler && scriptElements.length) {
            orderScriptElements(scriptElements);
            await addScriptElementsToScriptBundler(scriptElements, currentBundler);
