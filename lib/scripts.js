...
async function processScriptElements(markupProcessData) {
    ...
    if (insertPosition === options.OptionConstants.LOCAL) {
        ...
        // There should be max one bundler
...
function getScriptElements(markupProcessData) {
    ...
    while (true) {
        ...
        if (isAsync) { // Skip async scripts and leave them in markup
            if (!util.urlIsAbsolute(src)) {
                let file = fileUtils.navigateFromDirectoryWithRelativePath(markupProcessData.file.parent, src);
...
async function rollupFile(file, bundler) {
    bundler.log("Resolving module script...");
