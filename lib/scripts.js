...
async function processScriptElements(markupProcessData) {
    ...
    if (markupProcessData.options.bundleScript) await bundleScripts(markupProcessData);
    else await labelNecessaryScriptFiles(markupProcessData);

    // Regardless of the bundling option, insert the polyfill if specified
    if (markupProcessData.options.insertPolyfill) insertPolyfill(markupProcessData);
}
