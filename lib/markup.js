...
async function processMarkup(file, bundler) {
    ...
    if (bundler.options.bundleCss) await style.processStyleElements(markupProcessObject);
    if (bundler.options.bundleScripts) await scripts.processScriptElements(markupProcessObject);
...
function minifyMarkup(obj) {
    obj.bundler.log(`Minifying "${obj.file.getRelativePath()}"...`);
