...
async function processMarkup(file, bundler) {
    ...
    if (bundler.options.bundleCss) await style.processStyleElements(markupProcessData);
    if (bundler.options.bundleScript) await scripts.processScriptElements(markupProcessData);
