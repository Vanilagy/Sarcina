...
class MarkupProcessData {
    constructor(file, bundler) {
        ...
        this.options = this.bundler.options; // The options local to this markup file

        // Override the options if specified
        let optionOverride = options.findMatchingOverride(this.file, this.options.markupOptionOverride);
        if (optionOverride) {
